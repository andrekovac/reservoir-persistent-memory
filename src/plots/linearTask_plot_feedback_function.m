...
% clear all;
% 
% load('.\simulation_results\stereotyp_linear_nonlinearTask_WITH_fdb_data.mat');
...
% --------------------- plot parameters ----------------------
figure(4)
set( 4, 'Position', [ scrsz(3)/5    170         879       300         ] );
%                   [ left          bottom      width               height ]

% Problem with dynamic width:
% scrsz(3)*0.55 = 924 pixels at uni bamberg
% 1054 pixels = uni osnabrueck and heriot-watt university screens for tasks
% here for feedback: 879 pixels
...
plot( time , target_tf_vl( taskNr , time ), 'Color', [ 36/255 92/255 211/255 ] , 'LineWidth', 1);
hold on
plot( time , prediction_tf_vl( taskNr , time ), 'Color', [ 1 92/255 23/255 ] , 'LineWidth', 1);
hold off


% ======================== THINGS TO CHANGE ==============================
title( '\bfFeedback - experiment 2', 'fontsize', 12 );           % ------------ title 
...
yMargin  = ( max( target_tf_vl( taskNr , time ) ) - min( target_tf_vl( taskNr , time ) ) ) / 10;
% ======================== ================ ==============================
...
%         min( target_tf_vl( taskNr , time ) ) - yMargin      max( target_tf_vl( taskNr , time ) ) + yMargin] );
axis( [ min( time ) - xMargin      max( time ) + xMargin     ...
        min( prediction_tf_vl( taskNr , time ) ) - yMargin      max( prediction_tf_vl( taskNr , time ) ) + yMargin] );
...
hold on
    ...
    plot( target_tf( taskNr , time ) , prediction_tf_tf( taskNr , time ) , '.', 'Color', [ 241/255 225/255 10/255 ] , 'LineWidth', 0.5);
    plot( target_tf_vl( taskNr , time ) , prediction_tf_vl( taskNr , time ) , '.', 'Color', [ 143/255 100/255 0 ] , 'LineWidth', 0.5);
hold off
